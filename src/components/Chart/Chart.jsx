  ...
  <AreaChart data={data}>
    <defs>
      <linearGradient id={`color-${dataKey[0]}`} x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stopColor={stopColor[0][0]} stopOpacity={0.5} />
        <stop offset="90%" stopColor={stopColor[0][1]} stopOpacity={0.3} />
      </linearGradient>
    </defs>
    <XAxis
      dataKey="timestamp"
      ...
      connectNulls={true}
      padding={{ right: 20 }}
      hide={Boolean(!isExpanded && axisHidden)}
    />
    <YAxis
    ...
    <Tooltip
      content={
        <CustomTooltip
          ...
          isStaked={isStaked}
          isPOL={isPOL}
          setActive={(active) => setActive && setActive(active || data && data[0] || null)}
        />
      }
    />
    <Area dataKey={dataKey[0]} stroke={stroke} strokeWidth={strokeWidth || 0} fill={`url(#color-${dataKey[0]})`} fillOpacity={1} />
  ...
  <AreaChart data={data}>
    <defs>
      {dataKey.map((value, index) => {
          return <linearGradient key={value} id={`color-${value}`} x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor={stopColor[index][0]} stopOpacity={0.5} />
              <stop offset="90%" stopColor={stopColor[index][1]} stopOpacity={0.3} />
          </linearGradient>
      })}
    </defs>
    <XAxis
    ...
    <Tooltip
      ...
      content={<CustomTooltip bulletpointColors={bulletpointColors} itemNames={itemNames} itemType={itemType} setActive={(active) => setActive && setActive(active || data && data[0] || null)} />}
    />
      {dataKey.map((value, index) => {
          return <Area
                  key={value}
...
const renderLineChart = (
  ...
  <LineChart data={data}>
    ...
    <Tooltip
      content={<CustomTooltip bulletpointColors={bulletpointColors} itemNames={itemNames} itemType={itemType} setActive={(active) => setActive && setActive(active || data && data[0] || null)} />}
    />
    <Line type="monotone" dataKey={dataKey[0]} stroke={stroke ? stroke : "none"} strokeWidth={strokeWidth || 0} color={color} dot={false} />;
    {renderExpandedChartStroke(isExpanded, expandedGraphStrokeColor)}
  </LineChart>
);
...
const renderMultiLineChart = (
  ...
  setActive,
    ...
    <Tooltip
      content={<CustomTooltip bulletpointColors={bulletpointColors} itemNames={itemNames} itemType={itemType} setActive={(active) => setActive && setActive(active || data && data[0] || null)} />}
    />
    <Line dataKey={dataKey[0]} stroke={stroke[0]} strokeWidth={strokeWidth || 0} dot={false} />;
    <Line dataKey={dataKey[1]} stroke={stroke[1]} strokeWidth={strokeWidth || 0} dot={false} />;
    <Line dataKey={dataKey[2]} stroke={stroke[2]} strokeWidth={strokeWidth || 0} dot={false} />;
    <Line dataKey={dataKey[3]} stroke={stroke[3]} strokeWidth={strokeWidth || 0} dot={false} />;
    {renderExpandedChartStroke(isExpanded, expandedGraphStrokeColor)}
  </LineChart>
);
