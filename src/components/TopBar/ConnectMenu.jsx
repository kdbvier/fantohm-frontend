...
import { NetworkIds, networks } from "src/networks";

function ConnectMenu({ networkId }) {
  ...
  const primaryColor = networkId === NetworkIds.Moonriver ? "#49A1F2" : "#F8CC82"; // TODO add this to themes
  const buttonStyles =
    "pending-txn-container" + (isHovering && pendingTransactions.length > 0 ? " hovered-button" : "");

  const getEtherscanUrl = networks[chainId].getEtherscanUrl;
