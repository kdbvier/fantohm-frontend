...
const PoolTogether = () => {
  ...
  const { connect, address, chainId, hasCachedProvider } = useWeb3Context();
  const dispatch = useDispatch();
  let history = useHistory();
  const [graphUrl, setGraphUrl] = useState(networks[chainId].poolGraphUrl);
  ...
  useEffect(() => {
    setGraphUrl(networks[chainId].poolGraphUrl);
  }, [chainId]);
  ...
  useEffect(() => {
    ...
    if (walletChecked) {
      dispatch(getPoolValues({ networkId: chainId }));
      dispatch(getRNGStatus({ networkId: chainId }));
    }
  }, [walletChecked]);
