...
function BondPurchase({ bond, slippage, recipientAddress }) {
  ...
  useEffect(() => {
    dispatch(calcBondDetails({ bond, value: quantity, networkId: chainId }));
  }, [bondDetailsDebounce]);
          ...
          <>
              ...
              <div className="help-text">
                <em>
                  <Typography variant="body1" align="center" color="textSecondary">
                    First time bonding <b>{bond.displayName}</b>? <br /> Please approve Fantohm to use your{" "}
            ...
            {!bond.isAvailable[chainId] ? (
              ...
              <Button
                ...
                onClick={onBond}
              >
                {txnButtonText(pendingTransactions, "bond_" + bond.name, bond.isFhud ? `${bond.bondAction} ${bond.paymentToken}` : bond.bondAction)}
              </Button>
            ) : (
              <Button
                variant="contained"
        ...
        <Box className="bond-data">
          <div className="data-row">
            ...
            <Typography>
              {isBondLoading ? (
                ...
                <>
                  {trim(bond.balance, displayUnits === "LP" ? 18 : 4)} {displayUnits}
                </>
              )}
            </Typography>
          </div>
          ...
          <div className={`data-row`}>
            ...
            <Typography id="bond-value-id" className="price-data">
              {isBondLoading ? <Skeleton width="100px" /> : `${trim(bond.bondQuote, bond.paymentToken === "USDB" ? 2 : 4) || "0"} ${bond.paymentToken}`}
            </Typography>
          </div>
          ...
          <div className={`data-row`}>
            ...
            <Typography id="bond-value-id" className="price-data">
              {isBondLoading ? <Skeleton width="100px" /> : `${trim(bond.maxBondPrice, 4) || "0"} ${bond.paymentToken}`}
            </Typography>
          </div>
